...
const solution = (s: string) => s.split("").map((ch) => NATO[ch.toUpperCase()] ?? ch).join(" ");

export function EncodeLeit({ payload }: LeitViewProps) {
  const items: string[] = payload.items ?? [];
  return (
    <div className="space-y-4">
      <p className="text-sm text-slate-500">Wort/Rufzeichen normal vorlesen – der Trupp buchstabiert es zurück.</p>
      ...
      <details className="text-sm text-slate-600">
        <summary className="cursor-pointer">Lösung (zum Kontrollieren)</summary>
        <ol className="space-y-1 mt-2">
          {items.map((w, i) => (
            <li key={i}>
              <span className="text-slate-400">{i + 1}.</span> {solution(w)}
            </li>
          ))}
        </ol>
      </details>
...
export function EncodeTrupp({ config, onSubmit, submitting, lastResult }: TruppViewProps) {
  ...
  return (
    <div className="space-y-3">
      <div className="card space-y-2">
        <p className="text-sm text-slate-500">Gehörtes Wort buchstabieren (NATO-Wörter, durch Leerzeichen getrennt):</p>
        {Array.from({ length: count }, (_, i) => (
          <div key={i} className="flex items-center gap-2">
            <span className="text-slate-400 w-6 text-right">{i + 1}.</span>
            <input
              className="input"
              placeholder="Foxtrot Uniform November Kilo"
              disabled={locked}
              value={items[i] ?? ""}
...
export function EncodeConfigForm({ config, onChange }: ConfigFormProps) {
